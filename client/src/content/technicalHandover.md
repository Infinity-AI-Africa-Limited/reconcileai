...
```
  1. DATA COMES IN
     CSV upload  ·  API feed  ·  SFTP drop  ·  Core-banking connector (webhooks + daily sync)
        |
        v
  2. INGESTION & NORMALISATION      (dedupe · timestamp cleanup · field mapping)
        |
        v
  3. MATCHING ENGINE                (Pass 1 exact  ->  Pass 2 tolerance  ->  Pass 3 fuzzy)
        |
        +---------------------------+---------------------------+
                matched                                 unmatched
                  |                                         |
...
```
  USERS
    Bank / MFB staff        Infinity AI super admins        External systems
          |                          |                      (API clients, CBS)
          +------------+-------------+                              |
                       | typed tRPC calls (HTTPS)                   | REST + webhooks
                       v                                            v
  +==========================================================================+
  |  RECONCILEAI APPLICATION   (one Node.js app, deployed on Railway)        |
  |                                                                          |
  |  FRONTEND  — React 19 + Vite                                             |
  |     40+ pages: dashboards, exceptions, reports, admin, POC pages         |
  |                                                                          |
  |  BACKEND   — Express 4 + tRPC 11                                         |
  |     tRPC API  ·  Reconciliation engines (core / retail / mobile money /  |
  |     Woodcore)  ·  Exception taxonomies + Exception Intelligence          |
  |     (learning flywheel)  ·  Job queue (BullMQ)  ·  CBS connector engine  |
  |     ·  Auth (magic link)  ·  Public REST API + webhook endpoints         |
  +====+=========+=========+==========+==========+==========+================+
       |         |         |          |          |          |
       v         v         v          v          v          v
    TiDB /     Redis     S3 / R2   Anthropic   Resend    Partner core-banking
