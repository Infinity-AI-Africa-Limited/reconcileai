...
const SSO_ERROR_MESSAGES: Record<string, string> = {
  ...
  sso_not_allowed: "This account signs in with an email link — enter your email below.",
  sso_not_enabled:
    "Your organisation hasn't enabled Google/Microsoft sign-in yet. Use the email link below, or ask your administrator to request SSO.",
      ...
      <div className="bg-white rounded-2xl shadow-2xl p-10 max-w-md w-full space-y-6">
        ...
        {submitted ? (
          ...
          <>
            ...
            {/* Enterprise SSO — secondary option; shown only when at least one
                client organisation has requested Google/Microsoft sign-in. */}
            {ssoProviders.length > 0 && (
              <div className="space-y-3">
                <div className="flex items-center gap-3">
                  <div className="h-px flex-1 bg-gray-200" />
                  <span className="text-xs text-gray-400">or, if your organisation uses it</span>
                  <div className="h-px flex-1 bg-gray-200" />
                </div>
                {ssoProviders.map((p) => (
                  <Button
