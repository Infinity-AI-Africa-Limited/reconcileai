...
import CbnReportModule from "@/components/CbnReportModule";
...
export default function CBNCompliance() {
  ...
  const signAttestationMutation = trpc.cbnCompliance.signAttestation.useMutation();
  ...
  // ── Print attestation ──────────────────────────────────────────────────────
  // Digitally sign the attestation content (Ed25519) before printing so the
  // "timestamped, digitally signed" statement on the document is literally true.
  async function handlePrintAttestation() {
    try {
      const sig = await signAttestationMutation.mutateAsync({
        institution: institutionName,
        reportingPeriod: getPeriodLabel("Monthly"),
        overallStatus,
      ...
      setAttestationSig(sig);
    } catch {
      setAttestationSig(null);
      toast.error("Could not sign attestation — printing unsigned copy.");
    }
    // Let React paint the signature block before invoking the print dialog.
  ...
  return (
    <>
      ...
      <div id="cbn-attestation-print" ref={attestationRef} style={{ fontFamily: "serif", padding: "40px 60px", color: "#000", background: "#fff" }}>
        ...
        {attestationSig && (
          <div style={{ marginTop: "28px", border: "1px solid #1B365D", borderRadius: "6px", padding: "12px 16px", background: "#f8fafc" }}>
            <div style={{ fontSize: "11px", fontWeight: "bold", color: "#1B365D", marginBottom: "6px", display: "flex", alignItems: "center", gap: "6px" }}>
              ✓ Digitally Signed — {attestationSig.algorithm}
            </div>
            <table style={{ width: "100%", fontSize: "10px", color: "#333", borderCollapse: "collapse" }}>
              <tbody>
                <tr><td style={{ padding: "2px 0", fontWeight: "bold", width: "150px", verticalAlign: "top" }}>Signed at (UTC):</td><td style={{ fontFamily: "monospace" }}>{new Date(attestationSig.signedAt).toISOString()}</td></tr>
                <tr><td style={{ padding: "2px 0", fontWeight: "bold", verticalAlign: "top" }}>Content hash (SHA-256):</td><td style={{ fontFamily: "monospace", wordBreak: "break-all" }}>{attestationSig.contentHash}</td></tr>
                <tr><td style={{ padding: "2px 0", fontWeight: "bold", verticalAlign: "top" }}>Signature (Ed25519):</td><td style={{ fontFamily: "monospace", wordBreak: "break-all" }}>{attestationSig.signature}</td></tr>
                <tr><td style={{ padding: "2px 0", fontWeight: "bold", verticalAlign: "top" }}>Signing key fingerprint:</td><td style={{ fontFamily: "monospace" }}>{attestationSig.signingKeyFingerprint}</td></tr>
              </tbody>
            </table>
            <div style={{ fontSize: "9px", color: "#666", marginTop: "6px" }}>
              Verify authenticity against the institution's published ReconcileAI public key. Any change to the content above invalidates this signature.
            </div>
          </div>
        )}
        <div style={{ marginTop: "24px", textAlign: "center", fontSize: "10px", color: "#888", borderTop: "1px solid #ccc", paddingTop: "8px" }}>
          Generated by ReconcileAI · {today} · {attestationSig
            ? "This document carries a cryptographic Ed25519 signature (above) and is tamper-evident."
      ...
      <div className="space-y-6 p-6 print:hidden">
        ...
        <Tabs defaultValue="reports" className="space-y-4">
          ...
          {/* ── Tab: Regulatory Reports (standalone module) ── */}
