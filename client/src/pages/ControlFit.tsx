...
export default function ControlFit() {
  ...
  // Edits are counted, and the count at submit time is remembered, so a save
  // completing can tell "nothing changed while I was in flight" from "the
  // operator kept typing". Clearing `dirty` unconditionally on success re-armed
  // hydration below and silently replaced whatever had been typed during the
  ...
  const save = trpc.controlFit.save.useMutation({
    onSuccess: () => {
      ...
      if (editSeq.current === submittedSeq.current) setDirty(false);
      toast.success("Control Fit Brief saved");
      // Safe either way: while `dirty` is still set, the effect below refuses to
      // overwrite, so a refetch cannot clobber edits it does not know about.
      void brief.refetch();
    },
  });
  ...
  const submit = () => {
    submittedSeq.current = editSeq.current;
    return save.mutate({ ...form, ...scope, approvedEvidence: form.approvedEvidence.split("\n").map((value) => value.trim()).filter(Boolean) });
  };
