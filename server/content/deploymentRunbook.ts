...
The script ships in the image, so there is nothing to paste or maintain. It refuses to run unless \`DEPLOYMENT_MODE=on_premise\` — which both on-premise compose stacks set — because it grants platform administrator rights against whichever database \`DATABASE_URL\` names.
