...
| `shopify_shop_redaction_jobs` | `tenant_required` | Short-lived, durable work record for a verified `shop/redact` request. The current report-only gate records only scoped numeric inventory and ends in `blocked_dependency`; it emits no destructive completion, deletion proof, or erased claim. A future deletion-capable worker must replace—not bypass—this gate after approved manifest, global revocation, legal-retention, storage, key, and rehearsal controls are evidenced. |
