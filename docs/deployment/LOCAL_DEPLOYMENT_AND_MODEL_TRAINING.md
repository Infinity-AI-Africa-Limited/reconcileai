...
console. **`scripts/bootstrap-admin.mjs` ships in the repository and is copied
into the runtime image** (see `deploy/on-prem/Dockerfile`) — you do not create it.

> ⚠️ **This document used to embed a full copy of that script for you to paste.**
> That copy was removed, because it was a second implementation that could not
> be kept in step with the real one — and it drifted: the shipped script now
> refuses to run unless `DEPLOYMENT_MODE=on_premise`, since it mints a
> `super_admin` and prints a working sign-in link, and its only other condition
> is whichever database `DATABASE_URL` names. A pasted copy carried no such
> check. Run the file that ships; do not reproduce it.

> The script's column names match `drizzle/schema.ts` exactly (`organizations`,
> `users`, `magic_link_tokens`). These three tables are on the project's "do not
> change" list, so the raw-SQL approach is safe and version-stable. After the first admin
