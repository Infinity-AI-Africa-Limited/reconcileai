...
| `/payments/store/balance.json` | read_payment | — | `pending_settlement_balance`, `payout_account_available/frozen/balance`, `fixed_margin_account_balance`, `revolving_margin_account_balance`, `currency` |
| `/payments/store/merchants.json` | read_payment | — | `merchants[]`: `merchant_id`, `entity_name`, `entity_country` |
| `/store.json` (store information) | read_store_information | — | `id`, `merchant_id`, `name`, `domain`, **`currency`**, **`iana_timezone`**, `language`, `biz_store_status`, `email` |
