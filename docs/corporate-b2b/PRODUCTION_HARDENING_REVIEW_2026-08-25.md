...
diagnosed against the first leg, reporting a shortfall the size of the legs nobody had seen. The fix
matched an identifier followed by a **known** list or range connector.

Eighth round found that fix was the wrong shape. Review named `;`, `:` and `or`; `plus`, `|` and a
newline bypassed it too. The set of characters a human might put between two invoice numbers is not
enumerable, and every omission from that allowlist failed **open** — straight to a wrong shortfall.
Three rounds had now reached the same hole by three different routes, which made the approach the
defect rather than the list.

So the question is inverted. Instead of "is there a separator I recognise?", `parseReference` now
asks **"is there an invoice-length number here that nothing accounts for?"** That set *is* bounded:
a reference only carries so many kinds of number — the invoice identifiers themselves, amounts, and
dates. Anything left after those are masked out may be another invoice leg, whatever sits between
them. `ACCOUNTED_NUMBER_PATTERNS` is still a list and still incomplete, but an omission there makes
an explained number look unexplained, which **declines a determinate receipt** rather than
quantifying a wrong one. The direction of failure is the whole point of the change.

It still does **not** expand the shorthand: `INV-2001-2005` is five invoices under one customer's
numbering and one invoice under another's, and guessing would trade one wrong answer for another.
Establishing that more than one invoice may be named is all a function whose job is to decline needs.

> **Residual, stated rather than implied.** The rule requires at least one extracted identifier, so
> it reads as "more than the ones I found". A reference with no recognisable identifier at all — a
> bare `"1001 and 1002"` — is not covered: those digits are indistinguishable from an amount or an
> account number. The single-candidate branch that would then apply rests on a different argument
> (one open invoice is unambiguous), not on reading the reference.

> Both directions are proven, because a guard that cannot fail is not evidence. Deleting the guard
> fails all 13 refusal cases; deleting `ACCOUNTED_NUMBER_PATTERNS` fails all 8 controls — stated
> deduction amounts, thousands separators, decimals, currency cues, balance cues and both date
> shapes. An earlier fixture reused one candidate pool across cases, so the range case was refused
...
| Full suite | 2138 passing (was 1977); CI green on Tests, Typecheck & Build, Greptile Review |
