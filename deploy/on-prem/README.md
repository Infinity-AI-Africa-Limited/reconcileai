...
| CPU | 3B Q4 | 8–12 GB | ~3 GB | **p50 58s · p90 119s** on the reference laptop | see the measurement below |
| GPU | Qwen 8B+ | Size after capacity test | Model + cache | not measured | private vLLM serving only |

> **Measured, on one host, once — read the caveat before quoting these.**
> 36 held-out cases against the trained 3B Q4 model, on the reference laptop
> (HP EliteBook, Docker Desktop, CPU only, no other load):
>
> | | |
> |---|---|
> | latency | min 32s · **p50 58s** · p75 73s · **p90 119s** · max 1480s |
> | output | 136–214 tokens, median 177 |
> | throughput | ~2.3 tok/s at the median, 0.13 tok/s in the worst case |
> | completion | **36/36 finished cleanly** (`finish_reason=stop`), none truncated |
>
> The earlier "~2–4 s" and "~4–8 s" in this table were estimates and were wrong
> by roughly an order of magnitude at the median.
>
> **The tail is the host, not the model.** Three cases took 164s, 421s and
> 1480s while producing the same ~190 tokens as everything else — the same work
> at 1/20th the rate. A model generating more would show more tokens; these
> show stalling, which on a laptop under Docker Desktop means contention,
> thermal throttling or memory pressure. Do not carry the 1480s figure into a
> capacity plan; do not assume it disappears on server hardware either. Measure
> on the target host.
>
> A laptop is a demonstration host. These numbers exist so the table is not
> empty and not fiction — they are not a capacity benchmark, and week 3–4 of
> [`ACCELERATED_DUAL_TIER_EXECUTION.md`](../../docs/deployment/ACCELERATED_DUAL_TIER_EXECUTION.md)
> is where the institution's own figures come from.
