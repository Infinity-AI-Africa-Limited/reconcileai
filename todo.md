...
## Typechecking the test suite
- [x] Set `target: ES2022` in `tsconfig.json`. It was unset, so TypeScript defaulted to ES5 and iterating a `Set` or a RegExp iterator was an error — despite `lib` being esnext and the runtime being Node 22. Production code happened not to do it, so `pnpm check` stayed green and the misconfiguration was invisible. Verified production still checks clean; affects checking only, since Vite and esbuild build the bundles.
- [ ] Remove `"**/*.test.ts"` from the `tsconfig.json` exclude so CI typechecks test files. **66 errors remain** after the target fix (was 92): 54 TS2345, 6 TS2322, 3 TS2339, 2 TS2739, 1 TS2741 — concentrated in `server/reconciliation.test.ts` (42), `docs-s3.test.ts` (10), `readScopeRatchet.test.ts` (9). Mostly mock objects and bare strings where a union is expected. **Fix them by correcting the fixtures, not by casting**: a cast produces a green build and no added safety, which is the failure this whole exercise exists to close. Until then no fixture in this repository is contract-checked by CI — which is why the `as any` in the benchmark fixture suppressed nothing.
...
- [x] Capture non-destructive signed live-request evidence for the configured customer-data GDPR endpoint using a controlled developer-store payload; no customer or store redaction was invoked. Shop-redaction remains intentionally excluded from live testing because it is destructive.
...
- [ ] Verify the super-admin-only post-OAuth retail portal hand-off reaches `SL_RECONCILEAI_DEV` in Settlement Monitor. The CODE repair is merged and deployed; what remains is the authenticated visual confirmation, which needs a signed-in session and is the same evidence gate 4 of the readiness assessment is waiting on.
...
- [x] Fix the production post-OAuth retail tenant-context persistence defect. Closed by PRs #89, #91 and #92, all merged and deployed; verified against `main` — the portal-scope wiring, the handoff rule in `client/src/lib/shoplinePortalHandoff.ts`, and the cross-tenant import audit are all present.
...
- [x] Re-run the production OAuth hand-off after deployment of the fail-closed portal guard: Settlement Monitor now remains in `SL_RECONCILEAI_DEV` and uses the correct Retail Commerce navigation context.
- [x] Resolve the ReconcileAI Dev Store connector activation/visibility gap: after PR #91 deployment, the active `reconcileai-dev` store and recent successful sync are visible within the correct retail Settlement Monitor portal.
- [x] Fix Settlement Monitor server-side organisation scoping: the deployed `syncStatus` now reads the authorised `SL_RECONCILEAI_DEV` portal scope instead of the underlying Infinity AI Staff organisation.
...
- [x] Verify the deployed server-side portal-scope repair returns the active `reconcileai-dev` store and its actual sync state inside the `SL_RECONCILEAI_DEV` Settlement Monitor.
- [x] Submit the unmerged server-side portal-scoping commit for review. It became PR #91, merged; `resolveOrgScope` wiring is present in `main` (13 call sites) and every override-taking procedure authorises before touching the database.
- [x] Verify PR #91’s deployed portal-scoping repair renders the active ReconcileAI Dev Store and its real sync health inside the persisted `SL_RECONCILEAI_DEV` portal.
- [x] Create one synthetic ReconcileAI Dev Store cash-on-delivery source order for the P0 paid-order test: developer-store order #1004; no real payment method or customer data used.
- [x] Mark synthetic developer-store order #1004 paid through the authorised Cash on Delivery admin workflow; no real payment was taken.
- [x] Verify synthetic order #1004’s received `orders/paid` delivery: its processed webhook sequence and tenant-scoped order ingestion are confirmed. A native payment-leg row is correctly unavailable for Cash on Delivery without SHOPLINE Payments.
- [x] Diagnose the Cash on Delivery payment-leg boundary: the missing `sl_payments_reconcileai-dev` row reflects SHOPLINE’s optional Payments product, so Tier 1 must use the documented settlement-file fallback rather than fabricate a gateway payment row.
- [x] Import one controlled synthetic settlement file for paid developer-store order #1004 and verify the resulting matched order-to-remittance outcome in the retail Settlement Monitor.
- [x] Fix the settlement-file import tenant scope: after PR #92 deployment, column detection and import resolve the active `SL_RECONCILEAI_DEV` retail portal through the super-admin-only server guard.
- [x] Implement and validate the secure settlement-file portal scope repair: preview and commit now pass the active portal organisation through the existing super-admin-only `resolveOrgScope` guard; TypeScript and 1,831 tests pass.
- [x] Re-run the approved synthetic settlement-file import after PR #92 deployment and verify its preview, commit, and tenant-scoped reconciliation outcome for order #1004.
- [x] Re-run the approved synthetic settlement-file import after PR #92 deployment: preview detected all expected columns and the one-row import completed in the `SL_RECONCILEAI_DEV` portal.
- [x] Verify the redacted post-import ledger for order #1004: the SHOPLINE order and synthetic remittance rows are both matched with reciprocal match IDs; no unsupported native payment-leg claim is made.
- [x] Execute a bounded manual-sync recovery for `reconcileai-dev` and verify idempotency: one order and zero native payments processed; the #1004 source/remittance match remained exactly two rows with no related exception.
...
- [x] Correct that assessment's gate list, which named three owner gates as the whole remainder and omitted two repository controls: the drill record's "no submission before completed live evidence", and the three unchecked subscription-lifecycle tasks above. A release owner following the page alone could have submitted while those controls still said not to.
- [x] Resolve the drill record's stale "blocked until PR #87 is deployed" line — #87 merged 2026-08-18T21:41:33Z and production has restarted on the merged code — while keeping the separate no-submission-without-live-evidence control in force.
- [x] Add the sixth gate the corrected list still missed — "submit only after Richard confirms the final submission package" (line 1338 below). The correction had been assembled from this PR's own diff rather than the whole file, which is the same mistake in a smaller form; review caught it.
- [x] Make the readiness page's "Remaining owner actions" require all six gates. Step 3 said "only after the preceding decision is documented", referencing that list's own step 1 and silently omitting the repository controls, so following the actions in order could produce a submission while the drill record still prohibited one.
- [x] Remove the escape hatch that same fix introduced: step 3 had said gate 5 was satisfied by the lifecycle tasks closing "or the caveat formally accepted under gate 3", contradicting the page's own rule that gates 4–5 close by evidence rather than by decision. Accepting the caveat is now a recorded WAIVER of gate 5 — written into `RELEASE_CONTROL_RECORD.md` naming what is missing and confirmed under gate 6 — not gate 5 being met.
- [x] Remove the waiver path too. Reading `P2_DRILL_EXECUTION_RECORD.md` rather than reasoning from the summary showed it classes the lifecycle proof as **required P0 proof** in its own right, inside the record gate 4 protects — so a waiver written on the readiness page would authorise exactly what gate 4 forbids. Gate 3 now has two honest outcomes: pursue the supported billing-test route, or amend the control in the drill record itself.
...
- [x] Complete ReconcileAI-owned pre-bank readiness work: dependency evidence, session/CSRF design, durable-queue drill harness, storage-policy decision, and bank-pilot evidence-pack templates.
