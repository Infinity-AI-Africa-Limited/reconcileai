...
- [ ] Enter the blank Partner Portal contact name, monitored support email, customer-data GDPR endpoint, and store-data GDPR endpoint; then retain signed request evidence for each configured GDPR route.
- [ ] Complete live ReconcileAI Dev Store evidence: install/reinstall, OAuth callback, 90-day backfill, subscription lifecycle topic delivery, webhook duplicate/recovery handling, and a real test order reconciled in Settlement Monitor.
- [ ] Produce and upload the public App Store logo, real developer-store screenshots, and reviewer test guide; retain the Redirected loading mode and accurate non-SHOPLINE-Payments settlement-file limitation in the listing.
